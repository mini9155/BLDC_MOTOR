...
class QTextBrowserLogger(logging.Handler):
    ...
    def emit(self,record):
        ...
        self.order : str = ""
        self.label : str = ""
        if(re.findall('SEND: 0x1 0x4',record.message)):
            self.order_list.append(record.message[18:21])
        elif(re.findall('RECV: 0x81 0x4',record.message)):
            self.label.setText(self.state_to_(record.message,self.order_list))

    def state_to_(self,message : str, order : list):
...
class MainWindow(QMainWindow):
    ...
    def runnung(self):
        ...
        if(self.running_cnt == 1):
            ...
            self.move_cnt += min_speed * (self.up_set_rpm/100)
            ...
            self.lb_locate.setText("작동 중....")
            self.move_cnt -= min_speed * (self.down_set_rpm/100)
            self.lb_move.setText(str(round(self.move_cnt,1))+"mm")
            if(self.move_cnt < -370):
                print("over")
                self.stop()
    ...
    def up(self):
        ...
        self.running_cnt = 1
        reg = self.client.write_register(address=121, value=self.up_set_rpm, slave=1)
    ...
    def down(self):
        ...
        if(self.brake_cnt == 1):
            ...
            self.break_value()
            off = self.client.write_register(address=120, value=0x0100, slave = 1)
        self.down_set_rpm = -self.down_set_rpm
    ...
    def rpm_set(self):
        self.set_cnt = 1
        self.up_set_rpm = self.sp_rpm.value()
        self.down_set_rpm = self.sp_rpm.value()
